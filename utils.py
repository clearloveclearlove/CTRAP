...
class RetainDataset(Dataset):
    ...
    def __init__(self, data_path: str, tokenizer: transformers.PreTrainedTokenizer, poison_ratio=None, sample_num=None,
                 benign_dataset=None, finetuning_guide_data_num=None, poison_data_start=5000):
        super().__init__()
        ...
        index = 0
        from datasets import load_dataset
        ds = load_dataset("data/ultrachat_200k", split="test_sft")
        for example in ds:
            messages = example["messages"]
            # ======================= Only single-turn chat data ======================= #
            if len(messages) < 2: continue

            instance = {}
            instance["output"] = messages[1]["content"]
            instance["instruction"] = messages[0]["content"]
            instance["input"] = ""
            list_data_dict += [instance]
            index += 1
            if sample_num:
        ...
        logging.warning("Formatting inputs...")
        logging.warning(str(len(self.input_ids)))
        prompt_input, prompt_no_input = PROMPT_DICT["prompt_input"], PROMPT_DICT["prompt_no_input"]
        sources = [
            prompt_input.format_map(example) if example.get("input", "") != "" else prompt_no_input.format_map(example)
            for example in list_data_dict
...
class HelpfulDataset(Dataset):
    """Dataset for supervised fine-tuning."""

    def __init__(self, tokenizer: transformers.PreTrainedTokenizer,
                 sample_num=None):
        super(HelpfulDataset, self).__init__()
        logging.warning("Loading helpful data...")
