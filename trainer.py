...
class BoosterAlignmentTrainer(Trainer):
    ...
    def get_harmful_dataloader(self, harmful_datast, batch_size=10) -> DataLoader:
        """
        Returns the training [`~torch.utils.data.DataLoader`].
    ...
    def init(self, harmful_datast, helpful_datast):
        self.clock = 0
        self.steps = 0
        if self.args.guide_data_num > 0:
        ...
        self.helpful_dataloader = self.get_harmful_dataloader(helpful_datast, batch_size=self._train_batch_size)
        self.helpful_data_iter = iter(self.helpful_dataloader)
    ...
    def sample_from_helpful(self):
        # Get a  batch
        try:
            batch = next(self.helpful_data_iter)
        except (StopIteration):
            # If the iterator is exhausted, create a new iterator
            self.helpful_data_iter = iter(self.helpful_dataloader)
            batch = next(self.helpful_data_iter)
    ...
    def pad_and_concatenate(self, tensor1, tensor2, pad_type):
        if pad_type == "input_ids":
            pad_value = self.tokenizer.pad_token_id
        elif pad_type == "attention_mask":
            pad_value = 0
        elif pad_type == "labels":
        ...
        if d1 < max_d:
            padding_a = (0, max_d - d1)
            a_padded = torch.nn.functional.pad(tensor1, padding_a, "constant", pad_value)
        else:
            a_padded = tensor1

        if d2 < max_d:
            padding_b = (0, max_d - d2)
            b_padded = torch.nn.functional.pad(tensor2, padding_b, "constant", pad_value)
        else:
            b_padded = tensor2

        result = torch.cat((a_padded, b_padded), dim=0)
        return result
    ...
    def training_step(
        ...
        # may change input due to mode change
        model.train()
        alignment_inputs = self._prepare_inputs(inputs)
        harmful_inputs = self.sample_from_harmful()
        harmful_inputs = self._prepare_inputs(harmful_inputs)

        helpful_inputs = self.sample_from_helpful()
        retain_inputs = {"input_ids":self.pad_and_concatenate(alignment_inputs["input_ids"],helpful_inputs["input_ids"],pad_type='input_ids'),
                         "attention_mask":self.pad_and_concatenate(alignment_inputs["attention_mask"],helpful_inputs["attention_mask"],pad_type='attention_mask'),
                         "labels":self.pad_and_concatenate(alignment_inputs["labels"],helpful_inputs["labels"],pad_type="labels")}
        inputs = self._prepare_inputs(retain_inputs)
        ...
        def step():
            ...
            print("adv loss: {:.4f}  align loss: {:.4f}".format(loss.item() - loss2.item(),loss3.item()))
            return loss3
...
class CTRAPTrainer(Trainer):
    ...
    def training_step(
        ...
        retain_inputs = {"input_ids":self.pad_and_concatenate(alignment_inputs["input_ids"],helpful_inputs["input_ids"],pad_type='input_ids'),
                         "attention_mask":self.pad_and_concatenate(alignment_inputs["attention_mask"],helpful_inputs["attention_mask"],pad_type='attention_mask'),
                         "labels":self.pad_and_concatenate(alignment_inputs["labels"],helpful_inputs["labels"],pad_type="labels")}
        retain_inputs = self._prepare_inputs(retain_inputs)

        error_token = self.tokenizer('an error')['input_ids'][-1]

        collapse = {
            k: torch.where(v != IGNORE_INDEX, torch.tensor(error_token, dtype=v.dtype), v) if k == "labels" else v
            for k, v in retain_inputs.items()
        }
        collapse_inputs = self._prepare_inputs(collapse)
        ...
        def step():
            ...
            # backward the collapse gradient after harmful perturbation
            with self.compute_loss_context_manager():
                loss2 = self.compute_loss(model, collapse_inputs)
            if self.use_apex:
                with amp.scale_loss(loss2, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
            else:
            ...
            # backward the alignment gradient
            with self.compute_loss_context_manager():
                loss3 = self.compute_loss(model, retain_inputs)
            if self.use_apex:
                with amp.scale_loss(loss3, self.optimizer) as scaled_loss:
                    scaled_loss.backward()
            else:
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    param.grad.data = param.grad.data + self.args.lamb * perturb_grads[name]

            print("align loss: {:.4f}  collapse loss: {:.4f}".format(loss2.item(),loss3.item()))
            return loss3
        loss = step()
        return loss.detach() / self.args.gradient_accumulation_steps
...
class NPOTrainer(Trainer):
    ...
    def init(self, harmful_datast, helpful_datast):
        ...
        self.steps = 0
        if self.args.guide_data_num > 0:
            self.harmful_dataloader = self.get_harmful_dataloader(harmful_datast, batch_size=self._train_batch_size)
            self.harmful_data_iter = iter(self.harmful_dataloader)
        self.statistic = 0

        self.helpful_dataloader = self.get_harmful_dataloader(helpful_datast,
                                                              batch_size=self._train_batch_size)
        self.helpful_data_iter = iter(self.helpful_dataloader)
        self.ref_llm = copy.deepcopy(self.model).eval()
    ...
    def training_step(
        ...
        helpful_inputs = self.sample_from_helpful()
        helpful_inputs = self._prepare_inputs(helpful_inputs)
        ...
        def step():
            ...
            with self.compute_loss_context_manager():
                # loss = self.compute_loss(model, inputs)
                # normal_loss = self.compute_loss(model, helpful_inputs)
                align_loss = self.compute_loss(model, alignment_inputs)

                # loss_sft = normal_loss + self.args.lamb * pos_loss

                neg_prob = self.compute_logps(model, harmful_inputs)
                with torch.no_grad():
                    ref_neg_prob = self.compute_logps(self.ref_llm, harmful_inputs)

                log_delta = -(neg_prob - ref_neg_prob)
                unlearn_loss = torch.log(torch.nn.functional.sigmoid(log_delta))

                loss = align_loss - self.args.rho * torch.mean(unlearn_loss)
            ...
            self.log({'Unlearning Loss': -self.args.rho * torch.mean(
                unlearn_loss).item(),
                      'Align Loss': align_loss.item(),
                      # 'Positive Loss': pos_loss.item(),
                      # 'Normal prob': -normal_loss.item(),
                      # 'Positive prob': -pos_loss.item(),
                      'Negative prob': torch.mean(neg_prob).item()})
